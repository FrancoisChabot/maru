...
static void create_instance(VulkanRenderer *renderer, uint32_t extension_count,
...
static VkShaderModule create_shader_module(VulkanRenderer *renderer,
...
query_swap_chain_support(VulkanRenderer *renderer, VkPhysicalDevice dev);
...
static QueueFamilyIndices find_queue_families(VulkanRenderer *renderer,
                                              VkPhysicalDevice dev);

void vulkan_renderer_init(VulkanRenderer *renderer, uint32_t extension_count,
                          const char *const *extensions) {
  ...
  if (renderer->ready) {
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      vkDestroySemaphore(renderer->device,
                         renderer->renderFinishedSemaphores[i], NULL);
      vkDestroySemaphore(renderer->device,
                         renderer->imageAvailableSemaphores[i], NULL);
...
void vulkan_renderer_on_resized(VulkanRenderer *renderer, uint32_t width,
                                uint32_t height) {
...
void vulkan_renderer_draw_frame(VulkanRenderer *renderer) {
  ...
  vkWaitForFences(renderer->device, 1,
                  &renderer->inFlightFences[renderer->currentFrame], VK_TRUE,
                  UINT64_MAX);
  ...
  if (vkBeginCommandBuffer(renderer->commandBuffers[renderer->currentFrame],
  ...
  vkCmdSetViewport(renderer->commandBuffers[renderer->currentFrame], 0, 1,
                   &viewport);

  VkRect2D scissor = {0};
  scissor.offset = (VkOffset2D){0, 0};
  scissor.extent = renderer->swapChainExtent;
  vkCmdSetScissor(renderer->commandBuffers[renderer->currentFrame], 0, 1,
                  &scissor);
  ...
  vk_check(vkQueueSubmit(renderer->graphicsQueue, 1, &submitInfo,
                         renderer->inFlightFences[renderer->currentFrame]),
...
static void create_instance(VulkanRenderer *renderer, uint32_t extension_count,
  ...
  vk_check(vkCreateInstance(&createInfo, NULL, &renderer->instance),
...
static void create_logical_device(VulkanRenderer *renderer) {
  QueueFamilyIndices indices =
      find_queue_families(renderer, renderer->physicalDevice);
  ...
  vk_check(vkCreateDevice(renderer->physicalDevice, &createInfo, NULL,
                          &renderer->device),
...
static void create_swap_chain(VulkanRenderer *renderer) {
  SwapChainSupportDetails swapChainSupport =
      query_swap_chain_support(renderer, renderer->physicalDevice);
  ...
  vk_check(vkCreateSwapchainKHR(renderer->device, &createInfo, NULL,
                                &renderer->swapChain),
           "failed to create swap chain");

  vkGetSwapchainImagesKHR(renderer->device, renderer->swapChain, &imageCount,
                          NULL);
...
static void create_image_views(VulkanRenderer *renderer) {
  renderer->swapChainImageViews = (VkImageView *)malloc(
      sizeof(VkImageView) * renderer->swapChainImageCount);
  ...
  for (uint32_t i = 0; i < renderer->swapChainImageCount; i++) {
    ...
    vk_check(vkCreateImageView(renderer->device, &createInfo, NULL,
...
static void create_render_pass(VulkanRenderer *renderer) {
  ...
  vk_check(vkCreateRenderPass(renderer->device, &renderPassInfo, NULL,
                              &renderer->renderPass),
...
static void create_graphics_pipeline(VulkanRenderer *renderer) {
  VkShaderModule vertShaderModule =
      create_shader_module(renderer, vert_glsl_spv, vert_glsl_spv_len);
  VkShaderModule fragShaderModule =
      create_shader_module(renderer, frag_glsl_spv, frag_glsl_spv_len);
  ...
  vk_check(vkCreatePipelineLayout(renderer->device, &pipelineLayoutInfo, NULL,
  ...
  vk_check(vkCreateGraphicsPipelines(renderer->device, VK_NULL_HANDLE, 1,
                                     &pipelineInfo, NULL,
                                     &renderer->graphicsPipeline),
...
static void create_framebuffers(VulkanRenderer *renderer) {
  renderer->swapChainFramebuffers = (VkFramebuffer *)malloc(
      sizeof(VkFramebuffer) * renderer->swapChainImageCount);
  ...
  for (uint32_t i = 0; i < renderer->swapChainImageCount; i++) {
    ...
    vk_check(vkCreateFramebuffer(renderer->device, &framebufferInfo, NULL,
...
static void create_command_pool(VulkanRenderer *renderer) {
  ...
  vk_check(vkCreateCommandPool(renderer->device, &poolInfo, NULL,
...
static void create_command_buffers(VulkanRenderer *renderer) {
  ...
  vk_check(vkAllocateCommandBuffers(renderer->device, &allocInfo,
...
static void create_sync_objects(VulkanRenderer *renderer) {
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vk_check(vkCreateSemaphore(renderer->device, &semaphoreInfo, NULL,
                               &renderer->imageAvailableSemaphores[i]),
             "failed to create semaphore");
    vk_check(vkCreateSemaphore(renderer->device, &semaphoreInfo, NULL,
                               &renderer->renderFinishedSemaphores[i]),
             "failed to create semaphore");
    vk_check(vkCreateFence(renderer->device, &fenceInfo, NULL,
...
static void cleanup_swap_chain(VulkanRenderer *renderer) {
  for (uint32_t i = 0; i < renderer->swapChainImageCount; i++) {
    vkDestroyFramebuffer(renderer->device, renderer->swapChainFramebuffers[i],
                         NULL);
  }
  free(renderer->swapChainFramebuffers);

  for (uint32_t i = 0; i < renderer->swapChainImageCount; i++) {
    vkDestroyImageView(renderer->device, renderer->swapChainImageViews[i],
                       NULL);
  }
...
static VkShaderModule create_shader_module(VulkanRenderer *renderer,
  ...
  vk_check(
      vkCreateShaderModule(renderer->device, &createInfo, NULL, &shaderModule),
...
query_swap_chain_support(VulkanRenderer *renderer, VkPhysicalDevice dev) {
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(dev, renderer->surface,
                                            &details.capabilities);

  uint32_t formatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(dev, renderer->surface, &formatCount,
                                       NULL);
  ...
  uint32_t presentModeCount;
  vkGetPhysicalDeviceSurfacePresentModesKHR(dev, renderer->surface,
  ...
  if (presentModeCount != 0) {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        dev, renderer->surface, &presentModeCount, details.presentModes);
  }
...
static bool is_device_suitable(VulkanRenderer *renderer, VkPhysicalDevice dev) {
  ...
  if (extensionsSupported) {
    SwapChainSupportDetails swapChainSupport =
        query_swap_chain_support(renderer, dev);
...
static QueueFamilyIndices find_queue_families(VulkanRenderer *renderer,
  ...
  for (uint32_t i = 0; i < queueFamilyCount; i++) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(dev, i, renderer->surface,
                                         &presentSupport);
