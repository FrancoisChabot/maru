...
static std::vector<uint32_t> create_testbed_icon_pixels(uint32_t size) {
  ...
  for (uint32_t y = 0; y < size; ++y) {
    for (uint32_t x = 0; x < size; ++x) {
      ...
      pixels[y * size + x] = ((uint32_t)a << 24) | ((uint32_t)b << 16) |
...
static void SetupVulkan(ImVector<const char *> instance_extensions) {
...

  {
    ...
    pool_info.maxSets = 0;
    for (uint32_t i = 0;
         i < (uint32_t)(sizeof(pool_sizes) / sizeof(pool_sizes[0])); i++)
...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window *wd,
  ...
  wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(
      ...
      (int)(sizeof(requestSurfaceImageFormat) /
            sizeof(requestSurfaceImageFormat[0])),
...
int main(int, char **) {
  ...
  {
    App app(window, g_Instance, g_PhysicalDevice, g_Device, g_QueueFamily,
            g_Queue, g_DescriptorPool);
    g_App = &app;
