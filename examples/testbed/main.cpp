...
static void set_window_viewport_size(MARU_Window* window, MARU_Vec2Dip dip_viewport_size) {
  if (!window) return;
...
static void handle_maru_event(MARU_EventId type,
                              MARU_Window* window,
  ...
  if (g_App) g_App->onEvent(type, window, *event);
...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd,
  ...
  wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(
      ...
      (int)(sizeof(requestSurfaceImageFormat) / sizeof(requestSurfaceImageFormat[0])),
...
#endif
  wd->PresentMode =
      ImGui_ImplVulkanH_SelectPresentMode(g_PhysicalDevice, wd->Surface, &present_modes[0],
                                          (int)(sizeof(present_modes) / sizeof(present_modes[0])));

  IM_ASSERT(g_MinImageCount >= 2);
  ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, wd, g_QueueFamily,
                                         g_Allocator, width, height, g_MinImageCount, 0);
}
...
static void FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data) {
  VkSemaphore image_acquired_semaphore =
      wd->FrameSemaphores[(int)wd->SemaphoreIndex].ImageAcquiredSemaphore;
  VkSemaphore render_complete_semaphore =
      wd->FrameSemaphores[(int)wd->SemaphoreIndex].RenderCompleteSemaphore;
...
int main(int, char**) {
  ...
  while (g_KeepRunning && !g_WindowReady) {
    if (maru_pumpEvents(context, 16, MARU_ALL_EVENTS, handle_maru_event, NULL) != MARU_SUCCESS) {
  ...
  VkSurfaceKHR surface;
  if (maru_createVkSurface(window, g_Instance, vkGetInstanceProcAddr, &surface) != MARU_SUCCESS) {
  ...
  SetupVulkanWindow(wd, surface, (int)geometry.px_size.x, (int)geometry.px_size.y);
  ...
  {
    App app(window, g_Instance, g_PhysicalDevice, g_Device, g_QueueFamily, g_Queue,
            g_DescriptorPool);
    g_App = &app;
    ...
    while (g_KeepRunning) {
      ...
      const bool debounce_elapsed = g_PendingResize && (now >= g_LastResizeAtMs) &&
                                    ((now - g_LastResizeAtMs) >= g_ResizeDebounceMs);
      const bool need_rebuild = g_SwapChainRebuild || debounce_elapsed;
      if (geometry.px_size.x > 0 && geometry.px_size.y > 0 && need_rebuild) {
        const int target_width = (g_PendingResize && g_PendingViewportSize.x > 0)
                                     ? (int)g_PendingViewportSize.x
                                     : (int)geometry.px_size.x;
        const int target_height = (g_PendingResize && g_PendingViewportSize.y > 0)
        ...
        ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, wd,
                                               g_QueueFamily, g_Allocator, target_width,
                                               target_height, g_MinImageCount, 0);
