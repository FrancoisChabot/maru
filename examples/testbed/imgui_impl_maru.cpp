...
static void ImGui_ImplMaru_SetImeData(ImGuiContext*, ImGuiViewport* viewport, ImGuiPlatformImeData* data) {
    ...
    if (data->WantVisible) {
        ...
        attrs.dip_text_input_rect.position.x = (MARU_Scalar)local_x;
        attrs.dip_text_input_rect.position.y = (MARU_Scalar)local_y;
        attrs.dip_text_input_rect.size.x = (MARU_Scalar)1.0;
        attrs.dip_text_input_rect.size.y = (MARU_Scalar)line_h;
        maru_updateWindow(
...
void ImGui_ImplMaru_HandleEvent(MARU_EventId type, const MARU_Event* event) {
    ...
    if (type == MARU_EVENT_MOUSE_MOVED) {
        ...
        if ((event->window_state_changed.changed_fields & MARU_WINDOW_STATE_CHANGED_FOCUSED) != 0u) {
            io.AddFocusEvent(event->window_state_changed.focused);
        }
    }
}
