# Maru User Documentation

Welcome to the Maru user documentation. Maru is a high-performance, cross-platform library for windowing and event handling.

## Introduction & Getting Started

- [Installation & Integration](getting_started.md)
- [Basic Window Example](basic_window.md)

## Core Concepts

- [API Contracts](api_contracts.md)
- [Event Queues](queue.md)
- [Windows & Contexts](windows.md)
- [Input Handling](inputs.md)

## Advanced Features

- [Monitors & Displays](monitors.md)
- [Data Exchange (Clipboard/DnD)](dataexchange.md)
- [Vulkan Integration](vulkan.md)

## Backend-specific Guides

- [X11 Backend](X11.md)
- [Wayland Backend](wayland.md)
- [Windows Backend](windows_backend.md)
- [macOS Backend](macos_backend.md)
