...
def generate_helpers(xml_file, out_file):
    ...
    with open(out_file, 'w') as f:
        ...
        f.write("#ifndef MARU_WAYLAND_HELPERS_" + protocol_name.upper().replace('-', '_') + "_H\n")
        f.write("#define MARU_WAYLAND_HELPERS_" + protocol_name.upper().replace('-', '_') + "_H\n\n")
        f.write("#include <stdint.h>\n")
        f.write("#include <stddef.h>\n\n")
        
        # We rely on the context structure being defined before including this header
        f.write("typedef struct MARU_Context_WL MARU_Context_WL;\n\n")
        ...
        for interface in root.findall('interface'):
            ...
            # set_user_data
            write_func(f"maru_{iface_name}_set_user_data", "void", 
                       [f"MARU_Context_WL *ctx", f"struct {iface_name} *{iface_name}", "void *user_data"],
                       [f"ctx->dlib.wl.proxy_set_user_data((struct wl_proxy *) {iface_name}, user_data);"])
            
            # get_user_data
            write_func(f"maru_{iface_name}_get_user_data", "void *", 
                       [f"MARU_Context_WL *ctx", f"struct {iface_name} *{iface_name}"],
                       [f"return ctx->dlib.wl.proxy_get_user_data((struct wl_proxy *) {iface_name});"])

            # get_version
            write_func(f"maru_{iface_name}_get_version", "uint32_t", 
                       [f"MARU_Context_WL *ctx", f"struct {iface_name} *{iface_name}"],
                       [f"return ctx->dlib.wl.proxy_get_version((struct wl_proxy *) {iface_name});"])
            
            # add_listener
            write_func(f"maru_{iface_name}_add_listener", "int", 
                       [f"MARU_Context_WL *ctx", f"struct {iface_name} *{iface_name}", f"const struct {iface_name}_listener *listener", "void *data"],
            ...
            for request in interface.findall('request'):
                ...
                c_args = [f"MARU_Context_WL *ctx", f"struct {iface_name} *{iface_name}"]
                new_id_arg = None
                for arg in args:
                ...
                write_func(f"maru_{iface_name}_{req_name}", ret_type, c_args, body)

            # Default destroy if not already defined as a request
            write_func(f"maru_{iface_name}_destroy", "void", 
                       [f"MARU_Context_WL *ctx", f"struct {iface_name} *{iface_name}"],
