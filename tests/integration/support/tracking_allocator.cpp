...
MARU_IntegrationTrackingAllocator::find_allocation_nolock(
...
void *MARU_IntegrationTrackingAllocator::alloc_cb(size_t size, void *userdata) {
  ...
  std::lock_guard<std::mutex> lock(tracking->mutex_);
  return tracking->alloc_cb_nolock(size);
}

void *MARU_IntegrationTrackingAllocator::alloc_cb_nolock(size_t size) {
  alloc_event_count_++;
  if (oom_enabled_ &&
      alloc_event_count_ == oom_fail_at_event_) {
    oom_triggered_ = true;
    return nullptr;
  }
...
void *MARU_IntegrationTrackingAllocator::realloc_cb(void *ptr, size_t new_size,
  ...
  if (!ptr) {
    return tracking->alloc_cb_nolock(new_size);
  }

  if (new_size == 0u) {
    TrackedAllocation **prev_next = nullptr;
    TrackedAllocation *node = tracking->find_allocation_nolock(ptr, &prev_next);
    if (node && prev_next) {
      *prev_next = node->next;
