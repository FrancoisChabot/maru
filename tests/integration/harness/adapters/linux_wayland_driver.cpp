...
class LinuxWaylandDriver : public MARU_TestAutomationDriver {
 public:
  const char *name() const override { return "linux_wayland"; }
  ...
  bool inject_noop(MARU_TestSession *, char *reason,
                   size_t reason_capacity) const override {
    set_reason(reason, reason_capacity,
               "Wayland automation injection is not implemented yet in this "
               "harness.");
    return false;
  }
};
...
const MARU_TestAutomationDriver *maru_test_linux_wayland_driver() {
  static LinuxWaylandDriver driver;
  return &driver;
}
