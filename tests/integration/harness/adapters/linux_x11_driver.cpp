...
class LinuxX11Driver : public MARU_TestAutomationDriver {
 public:
  const char *name() const override { return "linux_x11"; }
  ...
  bool inject_noop(MARU_TestSession *, char *reason,
                   size_t reason_capacity) const override {
    set_reason(reason, reason_capacity,
               "X11 automation injection is not implemented yet in this "
               "harness.");
    return false;
  }
};
...
const MARU_TestAutomationDriver *maru_test_linux_x11_driver() {
  static LinuxX11Driver driver;
  return &driver;
}
