...
class MacOSDriver : public MARU_TestAutomationDriver {
 public:
  const char *name() const override { return "macos"; }
  ...
  bool inject_noop(MARU_TestSession *, char *reason,
                   size_t reason_capacity) const override {
    set_reason(reason, reason_capacity,
               "macOS automation injection is not implemented yet in this "
               "harness.");
    return false;
  }
};
...
const MARU_TestAutomationDriver *maru_test_macos_driver() {
  static MacOSDriver driver;
  return &driver;
}
